package com.flightticketsystem.runtime.filter;
...
public class SecurityFilter implements Filter {
            ...
            if (containsSuffix(uri) || GreenUrlSet.contains(uri) || containsKey(uri)) {
                ...
                for (int i = 0; i < cookies.length; i++) {
                    ...
                    if (cookie.getName().equals(Constant.LOGIN_SESSION_KEY)) {
                        ...
                        html = html.replace("_BP_", Constant.BASE_PATH);
    ...
    public String getRef(HttpServletRequest request) {
        ...
        request.getSession().setAttribute(Constant.LAST_REFERER, referer);
        return referer;
    }

    public String getUserId(String value) {
        try {
            String userId = Des3EncryptionUtil.decode(Constant.DES3_KEY, value);
            userId = userId.substring(0, userId.indexOf(Constant.PASSWORD_KEY));
            return userId;
        } catch (Exception e) {
